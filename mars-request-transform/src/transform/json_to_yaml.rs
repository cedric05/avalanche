...
#[derive(Clone)]
pub struct JsonTransformYAML<S> {
    inner: S,
    response_transform: bool,
    request_transform: bool,
}

pub struct JsonTransformYamlLayer {
    response_transform: bool,
    request_transform: bool,
}

impl<S> Layer<S> for JsonTransformYamlLayer {
    type Service = JsonTransformYAML<S>;
...
impl<S> Service<Request<ReqBody>> for JsonTransformYAML<S>
where
...
pub mod service_config {
    ...
    use super::JsonTransformYamlLayer;

    pub use mars_config::{TRANSFORM_JSON_TO_YAML_RESPONSE, TRANSFORM_JSON_YAML_REQUEST};

    impl TryFrom<&ServiceConfig> for JsonTransformYamlLayer {
        type Error = MarsError;

        fn try_from(value: &ServiceConfig) -> Result<Self, Self::Error> {
            let request_transform = value
                .params
                .get_value(TRANSFORM_JSON_YAML_REQUEST)
                .and_then(|x| x.as_bool())
                .unwrap_or_default();
            let response_transform = value
                .params
                .get_value(TRANSFORM_JSON_TO_YAML_RESPONSE)
                .and_then(|x| x.as_bool())
                .unwrap_or_default();
