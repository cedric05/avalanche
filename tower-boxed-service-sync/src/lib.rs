...
pub trait BoxSyncCloneServiceT<Request>: ServiceExt<Request> {
    fn boxed_sync_clone(self) -> BoxCloneSyncService<Request, Self::Response, Self::Error>
    where
        Self: Clone + Sized + Send + 'static + Sync,
...
impl<T, U, E> BoxCloneSyncService<T, U, E> {
    ...
    where
        S: Service<T, Response = U, Error = E> + Clone + Send + 'static + Sync,
...
where
    T: Service<R> + Send + Clone + 'static + Sync,
{
    fn clone_box(
        &self,
    ) -> Box<
